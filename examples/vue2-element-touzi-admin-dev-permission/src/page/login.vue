...
<script>import logoImg from "@/assets/img/logo.png";
...
export default {
   ...
   * @returns {{logo: unknown, loginForm: { username: string; password: string }, rules: { username: { required: boolean; message: string; trigger: string } | { min: number; max: number; message: string; trigger: string }[]; password: { required: boolean; message: string; trigger: string }[] }}}
   */
  data() {
    return {
      logo: logoImg,
    ...
    submitForm(loginForm) {
      this.$refs[loginForm].validate(valid => {
        if (valid) {
           ...
           * vue3-types TODO:
           * 
           *   - $router ×1: this.$router → useRouter()  (vue-router@4)
