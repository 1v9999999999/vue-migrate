            ...
            <el-menu 
                ...
                >
                    <template v-for="(item,index) in permission_routers">
                        ...
                        <!--表示 有二级或者多级菜单 -->
                        <el-sub-menu v-if="item.children  && item.children.length >= 1 && !item.hidden && !item.noDropdown"  :index="item.path" :key="index">
                            <template #title>
                               <template>
                                   <icon-svg v-if="item.meta.icon" :icon-class="item.meta.icon" />
                            ...
                            <router-link v-for="(citem,cindex) in item.children" :to="getIindex(citem,item,cindex)"  :key="cindex">
                                <el-menu-item 
                                    ...
                                    <template #title>
                                       <span> {{ $t(`commons.${citem.name}`)}}</span>
                                    </template>
                                </el-menu-item> 
                            </router-link>
                        </el-sub-menu>
...
export default {
   ...
   * @returns {{menuObj: { bgColor: string; textColor: string; activeTextColor: string }, logo: unknown}}
   */
  data() {
    return {
      menuObj: {
