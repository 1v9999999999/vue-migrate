...
const modalForm = reactive($form.createForm(this))
...
function onConfirmMethod() {
    ...
    onOk: () => ElMessage.success('OK'),
    onCancel: () => ElMessage.info('Cancel')
  });
}
function onInfoMethod() {
    this.$info({
    title: 'Info',
...
function onWarningMethod() {
    this.$warning({
    title: 'Warning',
    content: 'warning content'
  });
}
function onModalMethod() {
    this.$modal.confirm({
    title: 'Modal confirm',
...
function submitForm() {
    ...
    ElMessage.success(`提交: ${values.title}`);
    formModalVisible.value = false;
  });
}

watch(() => formModalVisible.value, (val) => {
    if (val) {
    nextTick(() => {
