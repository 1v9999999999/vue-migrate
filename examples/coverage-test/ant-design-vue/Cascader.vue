<template>
  <div class="cascader-demo">
    ...
    >
      <template slot="suffixIcon"><a-icon type="search" /></template>
    </a-select>
...
export default {
  ...
  data() {
    return {
          ...
          children: [
            {
              ...
              label: '杭州',
              children: [
                { value: 'xihu', label: '西湖区', isLeaf: true },
                { value: 'yuhang', label: '余杭区', isLeaf: true }
              ]
            },
          ...
          children: [
            {
              ...
              label: '南京',
              children: [
                { value: 'gulou', label: '鼓楼区', isLeaf: true }
              ]
            }
          ]
        }
      ...
      groupedOptions: [
        ...
        { label: '后端', options: [{ value: 'go', label: 'Go' }, { value: 'node', label: 'Node' }] }
      ],
      allCities: [
  ...
  methods: {
    onChange1(value, selectedOptions) {
      this.$message.info(`change1: ${value.join('/')}`)
    },
    onChange2(value, selectedOptions) {
      console.log('change2', value)
    },
    onMultiChange(value, selectedOptions) {
    ...
    loadData(selectedOptions) {
      ...
      setTimeout(() => {
        ...
        target.children = [
          { label: `${target.label} 动态 1`, value: `${target.value}-1`, isLeaf: true },
          { label: `${target.label} 动态 2`, value: `${target.value}-2`, isLeaf: true }
        ]
        this.lazyOptions = [...this.lazyOptions]
      }, 500)
    },
